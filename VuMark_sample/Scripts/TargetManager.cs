...
public class TargetManager : MonoBehaviour
{
    ...
    private Vector3 target1_pos, target1_rot;
    private Vector3 target2_pos, target2_rot;
    ...
    void Update()
    {
        ...
        if (track_rot)
        {
            target1.transform.rotation = Quaternion.Euler(target1.transform.rotation.eulerAngles + target1_rot);
            target2.transform.rotation = Quaternion.Euler(target2.transform.rotation.eulerAngles + target2_rot);
        }
        if (track_pos)
        {
            target1.transform.localPosition += target1_pos;
            target2.transform.localPosition += target2_pos;
        }
    }
