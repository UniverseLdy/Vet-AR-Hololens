        ...
        if ( m_trackirClient != null )
        {
            ...
            if ( bNewPoseAvailable )
            {
                if (First_time)
                {
                    ...
                    last_rot = poseOrientation.eulerAngles;
                    First_time = !First_time;
                }
                current_rot = poseOrientation.eulerAngles;
                current_pos = posePosition;
                Vector3 pos_error = current_pos - last_pos;
                Vector3 rot_error = current_rot - last_rot;
                // New data was available, apply it directly here.
                //transform.localPosition = posePosition;
                //transform.localRotation = poseOrientation;
                //cube.transform.localRotation = poseOrientation;
                DataManager.GetComponent<DataManager>().set_TrackIR(rot_error, pos_error);
