...
public class VuMarkTracking : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (!isHolo)
        {
            if (isBone)
            {
                path = @"D:\Dayu Li\Useful\backup projects\stage5\VuMark_sample\RoamingState\BoneData.txt".Replace('\\', System.IO.Path.DirectorySeparatorChar);
            }
            else
    ...
    public void OnVuMarkDetected(VuMarkTarget target)
    {
        ...
        //GameObject f_drill = GameObject.Find("drill");
        //f_drill.SetActive(true);
    }
    ...
    public void OnVuMarkLost(VuMarkTarget target)
    {
        ...
        */
        //StartCoroutine(delete_bone(target));

        //here we also need to move drill back to VuMark
        drill.transform.parent = drill_par.transform; //move drill back to its original parent, not delete by other method
    }
    ...
    void UpdateClosestTarget()
    {
        ...
        if (m_CurrentVuMark != null && m_CurrentVuMark == m_ClosestVuMark && counter % 30 == 0)
        {
            //Debug.Log(counter);
            foreach (var bhvr in m_VuMarkManager.GetActiveBehaviours())
            {
        ...
        if (counter % 30 == time_lag && update_flag == true)
        {
    ...
    //getting position of VuMark and create bone model as its child
    string GetStringVuMarkDescription(VuMarkTarget vumark)
    {
        ...
        foreach (var VuMark_object in m_VuMarkManager.GetActiveBehaviours(vumark))
        {
            ...
            else
            {
                ...
                Debug.Log("create a cylinder counter is " + counter); 
            }
            //assign to VuMark object
            created.transform.parent = VuMark_object.transform;
            created.transform.position = created.transform.parent.position; //why if I use VuMark_object.transform.position, the scale will change??? 
            */
            //created.transform.localPosition = new Vector3(0, 0.3f, 0);
            //created.transform.localEulerAngles = new Vector3(0, 0, 0);
            //change the scale of the object
            /*
            Vector3 rescale = created.transform.localScale;
            rescale.x = (float)0.5;
            rescale.y = (float)0.5;
            rescale.z = (float)0.8;
            created.transform.localScale = rescale;
            */
            ...
            if (VuMark_object != null)
            {
                ...
                if (LoadTextFile(vuMarkId, out model_data, out plane_data, out cones_data))
                {
                    ...
                    //create the drill -- later will move to cone creation part;
                    //put it under the cone prefab
                    //we set the drill her
                    /*
                    GameObject cone1 = GameObject.Find("Cone1");
                    drill.transform.parent = cone1.transform;
                    drill.transform.localPosition = new Vector3 (0,0,0);
                    drill.transform.localEulerAngles = new Vector3(0,0,0);
                    */
                    //find cone1
                    //drill.transform.parent = created.transform;
                    //drill.transform.localPosition = new Vector3(0,0.5f,0);
                    //GameObject cone1 = GameObject.Find("Cone1");
                    //drill.transform.localPosition = cone1.transform.localPosition;
                    //drill.transform.localEulerAngles = cone1.transform.localEulerAngles;
    ...
    bool LoadTextFile(string vu_name, out string model_data, out string plane_data, out string[] cones_data)
    {
        ...
        Debug.Log("PLANE DATA:" + plane_data + "!!!!!!!!!!");
        ...
        foreach (string line in lines) {
            Debug.Log("We are printing " + line);
            if (line.StartsWith(true_vu_name)) {
                //parse out the data part of this line
                raw_data_line = line.Split(':');
                data_line = raw_data_line[1];
                //print testing
                Debug.Log("The Data list is " + data_line);
                //seperate each data cell
                raw_data_list = data_line.Split(',');
                int counter = 0;
                foreach (string dt_cell in raw_data_list)
                {
                    data[counter] = Int32.Parse(dt_cell);
                    counter++;
                }

                if (counter != 3)
                {
                    Debug.Log("Length is not correct");
                }
                break;
            }

        }
        rotation = new Vector3(data[0], data[1], data[2]);
        Debug.Log(text);
        */
    }
