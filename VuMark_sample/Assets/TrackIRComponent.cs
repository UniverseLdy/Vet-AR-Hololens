...
public class TrackIRComponent : MonoBehaviour
{
    ...
    public Vector3 current_rot,current_pos;
    private Vector3 last_rot,last_pos;
    public GameObject DataManager;
    public GameObject created,cube;
            ...
            Quaternion poseOrientation = new Quaternion(
                ...
                if (bNewPoseAvailable)
                {
                    if (First_time)
                    {
                        ...
                        //TrackIR = (GameObject)Instantiate(cube);
                        //created.transform.rotation = Quaternion.Euler(new Vector3(0, 0, 0));
                        //created.GetComponent<MeshRenderer>().enabled = false;
                        //created.transform.parent = TrackIR.transform;
                        last_pos = posePosition;
                    First_time = !First_time;
                }
                //current_pos = posePosition;
                //Vector3 error = current_pos - last_pos;
                //TrackIR.transform.localPosition = posePosition;
                //TrackIR.transform.localRotation = poseOrientation;
                DataManager.GetComponent<DataManager>().set_TrackIR(poseOrientation, posePosition);
