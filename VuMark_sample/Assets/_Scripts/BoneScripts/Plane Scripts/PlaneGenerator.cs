...
public class PlaneGenerator : MonoBehaviour {
	...
	void Update () {
        ...
        // if mouse is clicked 
        if (Input.GetMouseButton(0) && mode.value == 1 && !(Input.GetKeyDown(KeyCode.RightAlt)) && !(Input.GetKeyDown(KeyCode.LeftAlt)))
        {
            RaycastHit hitInfo;
        ...
        if (Input.GetKeyDown(KeyCode.Delete) || Input.GetKeyDown(KeyCode.Backspace))
        {
            ...
            if (gizmo.SelectedObjects.Count != 0)
            {
                foreach(Transform p in gizmo.SelectedObjects)
                {
                    GameObject selected = p.gameObject;
    ...
    void createPlane()
    {
        ...
        newPlane.AddComponent<GizmoSelect>();
        
        // set to none mode
        mode.value = 0;

      //  Instantiate(plane, midpoint, newPlane.transform.rotation);
        planes.Add(newPlane);
    }
